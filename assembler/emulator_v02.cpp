...
void writeToFile();
void section2(vector<vector<string>> &parsedLines, vector<pair<string, string>> &instructions);
void section3(int &limit, bool trace, vector<pair<string, string>> &instructions, int &prevPC);
...
void section2(vector<vector<string>> &parsedLines, vector<pair<string, string>> &instructions)
{
    for (auto v : parsedLines)
    {
        ...
        {
            string binary = v[1];
            string opcode = binary.substr(5, 2);
    ...
    while (limit--)
    {
        ...
        if (trace)
            cout << instructions[PC].first << instructions[PC].second << endl
                 << " ";
        execute(instructions[PC].first + instructions[PC].second);
