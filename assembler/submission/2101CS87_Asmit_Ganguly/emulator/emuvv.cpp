...
file tree should be made as follows for proper working of the program
!
├───asm
├───assembler
├───emulator
...
├───o
└───trace
where the asm folder contains the .asm files and o folder contains the .o files generated
by the assembler. The emulator folder contains the emulator.cpp file and the log folder
contains the log files generated by the assembler. The l folder contains the .l files generated
by the assembler and the trace folder contains the trace files generated by the emulator.

#USAGE
1. To compile the emulator.cpp file use the following command
g++ emu.cpp -o emu
2. To run the emulator use the following command
.\emu.exe ../o/bubble.o -trace
